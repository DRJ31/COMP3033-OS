...
int main(void) {
	...
	while (1) {
		...
		DWORD promptLength = strlen(prompt); //The length of prompt
		WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE), prompt, promptLength, &promptLength, NULL);
		...
		DWORD bufSize = 1024; //The length of buf
		ReadConsole(GetStdHandle(STD_INPUT_HANDLE), buf, bufSize, &bufSize, NULL);
		...
		if (!CreateProcess(NULL, buf, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
			...
			DWORD errorMsgLength = strlen(errorMsg);
			WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE), errorMsg, errorMsgLength, &errorMsgLength, NULL);
			continue;
		}
